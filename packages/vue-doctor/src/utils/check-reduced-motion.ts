...
const REDUCED_MOTION_FILE_GLOBS = '"*.ts" "*.tsx" "*.js" "*.jsx" "*.vue" "*.css" "*.scss"';
...
const MISSING_REDUCED_MOTION_DIAGNOSTIC: Diagnostic = {
  ...
  help: "Add `useReducedMotion()` from @vueuse/core, or a `@media (prefers-reduced-motion: reduce)` CSS query",
  line: 0,
  column: 0,
...
const MOTION_LIBRARY_PACKAGES = new Set([
  "@vueuse/motion",
  "gsap",
  "@gsap/all",
  "@gsap/react",
