...
const detectFramework = (directory: string, dependencies: Record<string, string>): Framework => {
  // Check config files first for accurate framework detection
...
const extractDependencyInfo = (directory: string, packageJson: PackageJson): DependencyInfo => {
  ...
  return {
    ...
    hasPinia: Boolean(allDependencies["pinia"]),
    hasVueRouter: Boolean(allDependencies["vue-router"]),
  };
};
...
const findVueInWorkspaces = (
  ...
  for (const pattern of patterns) {
    ...
    for (const workspaceDirectory of directories) {
      ...
      if (info.hasPinia) result.hasPinia = true;
      if (info.hasVueRouter) result.hasVueRouter = true;
...
const findDependencyInfoFromMonorepoRoot = (directory: string): DependencyInfo => {
  ...
  if (!monorepoRoot)
    return { vueVersion: null, framework: "unknown", hasPinia: false, hasVueRouter: false };
  ...
  return {
    ...
    hasPinia: rootInfo.hasPinia || workspaceInfo.hasPinia,
    hasVueRouter: rootInfo.hasVueRouter || workspaceInfo.hasVueRouter,
  };
};
...
export const discoverProject = (directory: string): ProjectInfo => {
  ...
  const packageJson = readPackageJson(packageJsonPath);
  let { vueVersion, framework, hasPinia, hasVueRouter } = extractDependencyInfo(
    directory,
    packageJson,
  );
  ...
  if (!vueVersion || framework === "unknown") {
    ...
    if (workspaceInfo.hasPinia) hasPinia = true;
    if (workspaceInfo.hasVueRouter) hasVueRouter = true;
  }
  ...
  if ((!vueVersion || framework === "unknown") && !isMonorepoRoot(directory)) {
    ...
    if (monorepoInfo.hasPinia) hasPinia = true;
    if (monorepoInfo.hasVueRouter) hasVueRouter = true;
  }
