...
const CONFIG_DIRECTORY = join(homedir(), ".vue-doctor");
const CONFIG_FILE = join(CONFIG_DIRECTORY, "config.json");
const SKILL_REPO = "Arjun-Ingole/vue-doctor";
...
export const maybePromptSkillInstall = async (shouldSkipPrompts: boolean): Promise<void> => {
  ...
  logger.dim(
    `   Install the ${highlighter.info("vue-doctor")} skill to teach Cursor, Claude Code, Copilot,`,
  );
  logger.dim("   and other AI agents how to diagnose and fix these Vue issues.");
  logger.break();
